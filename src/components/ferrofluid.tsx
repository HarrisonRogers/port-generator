...
const prepColors = (input?: string[]) => {
  ...
  const arr: RGB[] = Array.from({ length: MAX_COLORS }, (_, i) => {
    const color = base[Math.min(i, base.length - 1)] ?? '#ffffff'
    return hexToRGB(color)
  })
  const avg: RGB = [0, 0, 0]
  for (let i = 0; i < count; i++) {
    const color = arr[i] ?? [0, 0, 0]
    avg[0] += color[0]
    avg[1] += color[1]
    avg[2] += color[2]
  }
...
const flowVec = (
  direction: FerrofluidProps['flowDirection'],
): [number, number] => {
  switch (direction) {
    case 'up':
      return [0, 1]
...
function Ferrofluid({
  ...
  useEffect(() => {
    ...
    if (mouseInteraction) {
      // Window listener so the magnet still works behind interactive content.
